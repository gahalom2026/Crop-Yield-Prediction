...
if df_raw is None:
    st.warning(
        "⚠️ `FAO_Crop_data.csv` was not found in this folder. Please upload it below "
        "(keep it alongside app.py so you don't need to upload it again next time)."
    )
...
# ============================================================
    ...
    with left:
        ...
        with sd1:
            state = st.selectbox("State", sorted(df["state"].dropna().unique()), key="predict_state")
        with sd2:
