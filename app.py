...
def render_predict():
    ...
    # Result is rendered here, right under the header, so it's visible
    # immediately after clicking Predict — no scrolling past the form needed.
    ...
    with st.container():
        ...
        with colA:
            ...
            rainfall = st.number_input(
                "Rainfall (mm)", min_value=0.0, max_value=5000.0,
                value=None, step=10.0, placeholder="Enter rainfall in mm",
            )
            temperature = st.number_input(
                "Temperature (°C)", min_value=-10.0, max_value=60.0,
                value=None, step=0.5, placeholder="Enter temperature in °C",
            )
            humidity = st.number_input(
                "Humidity (%)", min_value=0.0, max_value=100.0,
                value=None, step=1.0, placeholder="Enter humidity %",
            )
            crop = st.selectbox(
                "Crop", crop_classes, index=None, placeholder="Select a crop",
            )

        with colB:
            st.markdown("**Soil Conditions**")
            n = st.number_input(
                "Soil Nitrogen (N)", min_value=0.0, max_value=500.0,
                value=None, step=1.0, placeholder="Enter nitrogen level",
            )
            p = st.number_input(
                "Soil Phosphorus (P)", min_value=0.0, max_value=500.0,
                value=None, step=1.0, placeholder="Enter phosphorus level",
            )
            k = st.number_input(
                "Soil Potassium (K)", min_value=0.0, max_value=500.0,
                value=None, step=1.0, placeholder="Enter potassium level",
            )
            ph = st.number_input(
                "Soil pH", min_value=0.0, max_value=14.0,
                value=None, step=0.1, placeholder="Enter soil pH",
            )
