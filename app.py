...
if df_raw is None:
    st.warning(
        "⚠️ `FAO_Crop_data.csv` nahi mili is folder me. Neeche se upload kar do "
        "(app.py ke saath rakh degi to dobara upload nahi karna padega)."
    )
...
if missing_cols:
    st.error(
        "❌ Uploaded CSV expected schema se match nahi karta. "
        f"Ye columns missing hai: {missing_cols}"
    )
    st.stop()

if df_raw.empty:
    st.error("❌ Dataset khaali hai — kripya valid data wali CSV upload karo.")
    st.stop()
...
try:
    ...
    st.error(f"❌ Model training me error aaya: {e}")
    st.stop()
...
# ============================================================
    ...
    state_district_map = df.groupby("state")["district"].unique().apply(sorted).to_dict()
    ...
    with left:
        ...
        with st.form("predict_form"):
            ...
            with f1:
                ...
                district_options = state_district_map.get(state, sorted(df["district"].unique()))
    ...
    with right:
        ...
        if submitted:
            ...
            st.info("Form bharke **Predict Yield** dabao — result yaha dikhega.")
